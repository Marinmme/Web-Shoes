...
exports.productDetails_get_productDetail_byNew = (req, res, next) => {
  const paramNew = req.params.new;
  ProductDetail.find({new: paramNew})
    .populate({
      path: "product",
      populate: { path: "category" },
    })
...
exports.productDetails_get_QueryProductsByPrice = (req, res, next) => {
  const {from, to}= req.query;
  ProductDetail.find({price: {$gte: from, $lte: to}}).sort({price:1})
  .populate({
    path: "product",
    populate: { path: "category" },
  })
